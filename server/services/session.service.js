...
const videoCtrl = require('./twilio') 
...
function joinToSession(req, res, next) {
  ...
  Session.addUserToSession(courseId, userId)
    .then((data) =>
      VideoSession.getVideoSessionByCourseID(courseId, async (err, sessionData) => {
        if (sessionData) {
          // console.log(sessionData[sessionData.length -1]);
          let token = await videoCtrl.createSessionToken(sessionData[sessionData.length -1].roomName, userId);
          sessionData[sessionData.length -1].sessionToken = token;
          // console.log(sessionData);
          res.status(200).json({ success: true, message: "User Added to session", data: data, sessionData: sessionData })
        }
        else {
          res.status(500).json({ success: false, message: "error", data: err })
        }
      })
